...
| World-as-journey (7 `Ground`s walked in sequence: night → ignition → morning) | The page's whole-page colour decision is a *sequence*, not a single choice | A | This is a strictly larger version of the general pipeline's own `lib/design/worlds.ts` (`WorldId`), which today picks one static world. Generalizing means letting a world be a short ordered sequence — an extension of an abstraction that already exists, not a new one. |
...
| World-as-journey | YES (7 grounds) | NO (one static world) | NO | Partial (`ground()` helper already rebinds ink per-ground) | Extend `WorldId` to optionally be an ordered sequence |
