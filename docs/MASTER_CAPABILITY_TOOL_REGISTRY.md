...
3. **G-XAI-01** (RESOLVED 2026-08-25) — `xai` is fully wired and tested but bound to zero capabilities; the adapter cost was paid, the capability value was never realized. Bound to exactly one capability, `structured_generation` (order 3) — deliberately revisits a prior "KEEP unbound" verdict (five docs updated) with a concrete, evidenced case that verdict never examined: xai's native schema enforcement beats the already-bound Anthropic seat on cost and guarantee strength for this one capability.
4. **G-STAGE-01** (RESOLVED 2026-08-25) — three job-stage vocabularies still coexist; the unification module built to fix this has zero adopters. Audited as this entry asked: `state.ts` is NOT a strict superset (only 3/17 `jobState.ts` names and 1/23 `stage.ts` names match exactly) — its abstract states are meant to contain concrete stages as sub-steps, a real design task. Decision: keep the three as-is for now (each is currently load-bearing); `state.ts` stays the correct target model. Follow-up `WQ-019` queued with the mapping design + back-compat migration as its scope.
...
6. **G-BATTLE-01** (RESOLVED 2026-08-25, wired 2026-08-25 in a follow-up pass) — `lib/forge/battle.ts`'s multi-candidate Design Battle is fully implemented and now wired into the production entrypoint, opt-in via `cfg.forgeBattleMode`/`FORGE_BATTLE_MODE`, default off. `WORK_QUEUE.json` WQ-007 first decided NOT to wire it as the production default until three prerequisites closed: a per-candidate repair loop, budget-tier integration, and a `JobState` field distinct from the classic diverge battle's `designDirections`/`distinctnessScore`. `WQ-018` closed all three: `battle.ts` now runs `orchestrator.ts` step 9's exact repair loop per candidate; every candidate shares one `routing.capabilities` orchestrator instance so real spend is bounded across the whole battle, not per candidate; and `JobState.forgeBattle`/`summarizeForgeBattle` (`summary.ts`) are a dedicated field and read-model projection, never conflated with the diverge battle's fields. `runJob.ts`'s default build hook opts in via config; the single-build path (`runExperienceForge`) remains the unchanged default.
