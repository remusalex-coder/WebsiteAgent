...
|---|---|---|---|---|
| **gemini** | REAL_IMPLEMENTED | 12 (most-bound) | **none dedicated** | #1 executor for `reasoning` and `structured_generation`; licence class `free-tier-unverified` (O-3: commercial terms not settled) |
...
1. **G-AI-01** (high) — `gemini` and `anthropic`, the #1 executors for four of the six language capabilities, have zero dedicated behavioural test files. `groq` is the only provider with real fetch-stubbed coverage.
