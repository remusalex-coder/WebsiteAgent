...
6. **G-BATTLE-01** (RESOLVED 2026-08-25) — `lib/forge/battle.ts`'s multi-candidate Design Battle is fully implemented but unreachable from the production entrypoint. Deliberately decided (`WORK_QUEUE.json` WQ-007) NOT to wire it as the production default: it lacks a per-candidate repair loop (`runExperienceForge`'s step 9), lacks budget-tier integration, and has no `JobState` field distinct from the classic diverge battle's `designDirections`/`distinctnessScore`. Wiring it in unprepared would regress quality and corrupt the one-read-model control surface. Follow-up `WQ-018` queued with those three prerequisites as its scope.
