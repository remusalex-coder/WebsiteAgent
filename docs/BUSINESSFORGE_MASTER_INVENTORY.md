...
| Job state (`createJob`/`loadJob`/`saveJob`) | `lib/workflow/jobState.ts` | **USED / TESTED** | The actually-wired persistence path (both `runJob.ts` and `stage.ts` use it). Correction 2026-08-25 (WQ-006): `lib/workflow/state.ts` is **not** imported by `hermes.ts` or anywhere else in production — this row's prior claim was stale; a repo-wide grep for `from './state.js'`/`from '../workflow/state...'` finds only `test/state.test.ts`. See `WORK_QUEUE.json` WQ-006 for the full stage-vocabulary audit. |
...
| n8n Factory V1 | **REMOVED 2026-08-25** (`WORK_QUEUE.json` WQ-020) | done | Was `n8n/factory-v1.json` + `scripts/n8n/build-factory-workflow.ts`. Confirmed zero repo-internal runtime callers beyond two tests asserting its generated node shape, which were removed with it (the rest of those test files stayed — unrelated real coverage). `/stage/:name` itself stays; still real, used for manual curl debugging. |
...
### n8n
Covered in §2. One workflow file remains — the Control Surface (current). Factory V1 (legacy duplicate) was removed 2026-08-25, `WORK_QUEUE.json` WQ-020.
...
3. **RESOLVED 2026-08-25 (`WORK_QUEUE.json` WQ-006).** The duplication is real (audited); `state.ts`'s 12 abstract states are not a strict superset of `main.ts`'s 9 / `jobState.ts`'s 17 / `stage.ts`'s 23 concrete names, so "pick one and delete the others" is not a safe mechanical move — decided to keep all three for now, with a scoped follow-up (`WQ-019`) for the real concrete-to-abstract mapping design.
4. **RESOLVED 2026-08-25 (`WORK_QUEUE.json` WQ-020).** `n8n/factory-v1.json` and its generator removed; confirmed zero repo-internal dependents beyond two tests asserting its shape, which were removed with it. `/stage/:name` itself stays — real, used for manual curl debugging, independent of any n8n workflow.
