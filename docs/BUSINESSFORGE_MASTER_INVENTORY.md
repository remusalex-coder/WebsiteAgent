...
| xAI (Grok) | INTEGRATED, no capability bindings | Present as an adapter but `ALL_BINDINGS.some(b => b.provider === 'xai')` is `false` by design — "orphaned until deliberately wired," confirmed by a dedicated test. |
