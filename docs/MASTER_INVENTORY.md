...
| xAI (Grok) | REAL adapter, **one capability binding** (`structured_generation`, order 3) | `[VERIFIED]` 2026-08-25, WQ-005, dedicated test asserts exactly one binding | KEEP, bound narrowly — revisits the prior "no measured reason" verdict with one: xai's native schema enforcement is cheaper on both axes than the already-bound Anthropic seat for this one capability, the same fallback-depth logic already used for Cerebras/Groq elsewhere | High |
