...
| xAI | Grok | structured-output (`structured_generation`, order 3) | Fallback native-schema seat, cheaper than the already-bound Anthropic seat on both axes (200/600 vs 276/1380 cents/million) | Paid, no free tier, `priceConfidence: observed` | REAL/INTEGRATED/TESTED — bound 2026-08-25, WQ-005 | same | **KEEP the adapter, KEEP the one narrow binding** | Revisits this row's prior "KEEP it unbound" verdict: the exclusivity test never checked xai against one already-bound vendor on one specific capability — this binding uses the same fallback-depth reasoning already applied to Cerebras/Groq elsewhere in the pool, not a blanket wire-in |
