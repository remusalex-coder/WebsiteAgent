...
async function direct(
  ...
  logger: Logger,
  signal: AbortSignal,
): Promise<{ directive: DesignDirective; model: string }> {
  let result;
  try {
  ...
  return { directive: result.data as DesignDirective, model: result.model };
}
...
export const designDirectorAgent: DesignDirectorAgent = {
  ...
  async run(input: DesignDirectorInput, ctx: AgentContext): Promise<DesignDirective> {
    ...
    const { directive, model } = await logger.time('direct design', () =>
