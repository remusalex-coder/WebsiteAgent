 ...
 */
/*
 * Density and emphasis compose into one padding, not two rules that overwrite
 * each other.
 *
 * Both used to set \`padding-block\` directly, so a page whose density and
 * whose per-section emphasis disagreed only ever showed whichever rule the
 * cascade happened to prefer — in practice, emphasis never touched padding at
 * all beyond \`lead\`/\`quiet\`, which is why a \`primary\` section and a
 * \`secondary\` one sat in identical bands on a page whose base density was
 * \`balanced\`: nothing distinguished them. Each axis now only ever sets its
 * own custom property, and the one \`padding-block\` declaration multiplies
 * them — so every emphasis step leaves a visible mark on every density, not
 * only the two extremes.
 */
.section {
  padding-block: calc(var(--space-section) * var(--section-density-factor, 1) * var(--section-emphasis-factor, 1));
}

.section[data-density="airy"] {
  --section-density-factor: ${round(1.22 / DENSITY_FACTOR[personality.density])};
}

.section[data-density="balanced"] {
  --section-density-factor: ${round(1 / DENSITY_FACTOR[personality.density])};
}

.section[data-density="dense"] {
  --section-density-factor: ${round(0.78 / DENSITY_FACTOR[personality.density])};
  --grid-gap: var(--space-sm);
}

.section[data-emphasis="lead"] {
  --section-emphasis-factor: 1.14;
  --grid-gap: var(--space-lg);
}

/*
 * \`primary\` and \`secondary\` used to be visually the same section: neither
 * had a padding or type-size rule of its own, so both fell through to the
 * page's base density and the default \`h2\`. Two adjacent sections at
 * different emphasis are supposed to read as different weights in the
 * page's argument — a modest step each, so the ladder stays legible without
 * the interior steps competing with \`lead\`.
 */
.section[data-emphasis="primary"] {
  --section-emphasis-factor: 1.04;
}

.section[data-emphasis="secondary"] {
  --section-emphasis-factor: 0.95;
}

.section[data-emphasis="quiet"] {
  --section-emphasis-factor: 0.86;
  --grid-gap: var(--space-sm);
}
...
.section[data-emphasis="secondary"] .section__head h2 {
  font-size: calc(var(--text-h2-size) * 0.92);
}

.section[data-emphasis="primary"] .section__head h2 {
  font-size: calc(var(--text-h2-size) * 1.06);
}
...
.section[data-role="signature"]:not(.section--hero) .section__head h2 {
  /*
   * Capped as a fraction of the *hero's own* display size, not against a flat
   * rem value.
   *
   * A flat 6rem cap is a number with no relationship to this business's type
   * scale — on the mechanic benchmark it landed at 86px against a 49px hero,
   * which inverted the one hierarchy rule the narrative layer exists to keep:
   * the opening is the loudest thing on the page, the peak is unmistakably
   * second. \`--text-display-size\` is the same token the hero's own h1 rule
   * caps against, so the signature can never out-scale it, on any theme's
   * scale, at any viewport.
   */
  font-size: clamp(2.2rem, 6vw, calc(var(--text-display-size) * 0.82));
  line-height: 1.02;
...
/*
 * The signature beat, when it also bleeds, becomes a dark band.
 *
 * Bigger type alone reads as "the same page, louder". A ground change is what
 * makes a peak feel like an event rather than a heading that grew — so the one
 * section the narrative layer nominated as its climax, when it also has the
 * room a bleed section has, gets the page's darkest ground and the ink built
 * for it, the same pairing already proven on the inverted band and the
 * full-bleed hero.
 */
.section[data-role="signature"].section--bleed {
  ${ground('var(--color-inverted)', invertedInk)}
}
...
.statement__label h2 {
  ...
  font-family: var(--font-heading);
  /*
   * Floored above the raw eyebrow step.
   *
   * The eyebrow token is sized for a short line of type sitting directly under
   * a display heading, where the heading itself carries the section's weight.
   * Here the label *is* the heading — the only \`<h2>\` a section like "Opening
   * hours" has — and the raw token rendered it at 11px on a compact theme,
   * smaller than the small print in the footer. A heading a person has to
   * hunt for is a defect the "label, not a title" intent never asked for.
   */
  font-size: max(var(--text-eyebrow-size), 0.9rem);
  line-height: var(--text-eyebrow-height);
  font-weight: var(--weight-heading-max);
  letter-spacing: var(--text-eyebrow-tracking, 0.08em);
  text-transform: uppercase;
...
/* The section's one image still takes the role's own framing over the page default. */
.split__media img[data-framing="wide"] { aspect-ratio: 16 / 9; }
.split__media img[data-framing="tall"] { aspect-ratio: 3 / 4; }
.split__media img[data-framing="square"] { aspect-ratio: 1 / 1; }
...
.alternating__media img[data-framing="wide"] { aspect-ratio: 16 / 9; }
.alternating__media img[data-framing="tall"] { aspect-ratio: 3 / 4; }
.alternating__media img[data-framing="square"] { aspect-ratio: 1 / 1; }
...
/*
 * Art-directed composition, not a uniform contact sheet.
 *
 * Every image in a gallery used to share one crop and one treatment, which is
 * the single largest reason a generated page reads as a template with photos
 * dropped in rather than a set somebody edited: an environment shot, a
 * close-up detail and a photograph of somebody at work are different kinds of
 * picture and a real edit crops them differently. \`data-role\` and
 * \`data-framing\` come from the design's asset choreography
 * (\`lib/design/assets.ts\`) — the same evidence-driven decision that already
 * picks the hero and the signature moment, extended down to every image in
 * the set — so two businesses with different photographs, or the same
 * photographs in a different character, compose materially differently.
 *
 * Aspect ratio only, never crop position guessed from content the pipeline
 * cannot see: the point is rhythm across the set, not a claim about where the
 * subject sits in any one frame.
 */
.gallery img[data-framing="wide"] {
  aspect-ratio: 16 / 9;
}

.gallery img[data-framing="tall"] {
  aspect-ratio: 3 / 4;
}

.gallery img[data-framing="square"] {
  aspect-ratio: 1 / 1;
}

/* A close look at one thing sits slightly inset, like a print in a mount. */
.gallery__item[data-role="detail"] figure {
  padding: var(--space-2xs);
  background: var(--color-surface);
  border-radius: var(--image-radius);
}

.gallery__item[data-role="detail"] img {
  border-radius: calc(var(--image-radius) - var(--space-3xs, 0.25rem));
}

/* A person at work is framed from the shoulders up, not cropped through the middle. */
.gallery img[data-role="people"] {
  object-position: top;
}
...
/*
 * The section the narrative built to breaks the grid, not just the type
 * scale.
 *
 * Before this, "the signature beat" meant a bigger heading and a darker
 * band — the same card, louder. A peak in an edited set reads as a peak
 * because the composition itself changes: one photograph takes the room the
 * rest of the page never gives an image, everything around it becomes a
 * quieter counter-column, and the section around it breathes rather than
 * repeating the page's usual rhythm. \`section--signature-composition\` is
 * set once, in the renderer, from \`SectionDesign.role === 'signature'\` — the
 * same narrative decision that already promotes the heading — so whichever
 * kind of section this business's evidence made its peak (a gallery, a menu,
 * a story) gets this break.
 *
 * \`order: -1\` rather than reordering the DOM: the dominant cell is whichever
 * image the asset choreography actually chose as the signature
 * (\`data-role="signature"\`) when the section carries one, and the first
 * photograph otherwise — a deterministic fallback, not a guess, for a
 * signature section built from content that never named one image over the
 * rest.
 */
.section--signature-composition {
  padding-block: calc(var(--space-section) * var(--section-density-factor, 1) * 1.3);
}

.section--signature-composition .section__head {
  max-width: 40ch;
}

.section--signature-composition .gallery,
.section--signature-composition .gallery--masonry {
  display: grid;
  columns: initial;
  grid-template-columns: repeat(2, minmax(0, 1fr));
...
.section--signature-composition .gallery__item {
  break-inside: auto;
  margin-bottom: 0;
}

.section--signature-composition .gallery__item:first-child,
.section--signature-composition .gallery__item[data-role="signature"] {
  grid-column: 1 / -1;
  order: -1;
}

.section--signature-composition .gallery__item:first-child img,
.section--signature-composition .gallery__item[data-role="signature"] img {
  width: 100%;
  aspect-ratio: 16 / 9;
  height: auto;
}
...
@media (min-width: ${round(mdRem)}rem) {
  ...
  /* The rest of the set becomes a counter-column beside the dominant frame. */
  .section--signature-composition .gallery__item:first-child img,
  .section--signature-composition .gallery__item[data-role="signature"] img {
    aspect-ratio: 21 / 9;
  }
...
/* ================================================================== */
/* RECOVERY — spatial depth, layered grounds, veil peak, kinetic motion */
/* ================================================================== */
/*
 * These recover, inside the deterministic CSS-only renderer, the *principles*
 * the WebGL Bakery V2 engine demonstrated (docs/experience-capability-audit.md,
 * docs/canonical-bakery-v2.md) without reintroducing a runtime: depth and
 * layering, a genuine veil/whiteout at the signature moment, and cinematic
 * entrance + kinetic hover. Every rule is gated by prefers-reduced-motion and
 * (where it reads a hover) by (hover: hover), so the accessible, deterministic
 * floor is untouched. None of this touches a section's structure or its
 * existing tokens — it adds *stagecraft* on top of the composition the design
 * layer already chose. Keyed only on classes the renderer already emits
 * (.section--hero, .section--signature-composition, .section--moment),
 * so a business earns depth/motion precisely when its evidence already earned a
 * hero photo and a signature — no new design field, no contract change.
 */

/* --- R1: spatial depth & layered grounds on the image-led peaks -------- */
/* The hero and the signature are the two places a page has something to
 * reveal in depth. A perspective stage + a layered ground band behind the
 * media gives them a third dimension without a single line of JS. The
 * transform is inside the reduced-motion guard; under reduced motion the
 * section simply renders flat, which is the correct degradation. */
@media (prefers-reduced-motion: no-preference) {
  .section--hero {
    perspective: 1400px;
  }
  .section--hero .hero__media,
  .section--signature-composition .gallery__item[data-role="signature"] {
    transform: translateZ(0) rotateX(var(--depth-tilt, 1.2deg));
    transition: transform var(--duration-slow) var(--easing);
  }
  /* A layered ground band: a second plane behind the content, the way
   * Bakery's loaf sat above a graded world. ::after is decorative and
   * non-interactive by construction. */
  .section--hero::after,
  .section--signature-composition::after {
    content: "";
    position: absolute;
    inset: auto 8% -2.5rem 8%;
    height: 40%;
    background: radial-gradient(120% 100% at 50% 0%, color-mix(in srgb, var(--color-brand) 16%, transparent), transparent 70%);
    filter: blur(28px);
    opacity: 0.9;
    pointer-events: none;
    z-index: 0;
  ...
  .section--signature-composition > .container {
    position: relative;
    z-index: 1;
  }
}
...
@media (prefers-reduced-motion: no-preference) {
  ...
  .section--moment[data-transition="veil"]::before {
    ...
    position: absolute;
    inset: 0;
    background: var(--color-canvas, #fcfcfc);
    pointer-events: none;
    z-index: 2;
...
/* --- R3: cinematic entrance + kinetic hover --------------------------- */
/* Elevate the existing timid hover (transition only) into a transform that
 * lifts and scales, so interactive things have weight. And give the signature
 * and about beats a one-shot entrance so the page arrives as a sequence,
 * not a slab. All gated exactly like the hero entrance above. */
...
@media (prefers-reduced-motion: no-preference) {
  ...
  .section--signature-composition .gallery__item[data-role="signature"] {
    animation: forge-signature-rise var(--duration-slow) var(--easing) both;
  }
}
@keyframes forge-signature-rise {
